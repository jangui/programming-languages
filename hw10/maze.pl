...
addDirection(R, C, n, NewR, C) :- NewR is R - 1.
addDirection(R, C, s, NewR, C) :- NewR is R + 1.
addDirection(R, C, e, R, NewC) :- NewC is C + 1. 
addDirection(R, C, w, R, NewC) :- NewC is C - 1. 

newPos(OldRow,OldCol,Direction,NewRow,NewCol) :- 
  addDirection(OldRow, OldCol, Direction, NewRow, NewCol),
  not(mazeWall(NewRow,NewCol)),
  mazeDimension(MR, MC),
  NewRow =< MR,
  NewCol =< MC.
...
solve(R,C,NextR,NextC,VisitedIn,VisitedOut,MoveListIn,MoveListOut) :-
  move(R, C, NextR, NextC, VisitedIn, VisitedOut, MoveListIn, MoveListOut),
  mazeEndPos(NextR, NextC),!.
solve(R,C,NextR,NextC,VisitedIn,VisitedOut,MoveListIn,MoveListOut) :-
  move(R, C, NextR, NextC, VisitedIn, VisitOut, MoveListIn, MoveOut),
  solve(NextR, NextC, _, _, VisitOut, VisitedOut, MoveOut, MoveListOut).
